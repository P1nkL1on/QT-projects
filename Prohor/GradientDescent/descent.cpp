...
void Descent::Step()
{
    QVector<Derivable> getedDeriv = AutoDiff(modelOriginal, modelFinal, currentStep);
    QVector3D currentAdd = QVector3D(getedDeriv[0].getProiz(), getedDeriv[1].getProiz(), getedDeriv[2].getProiz());
    qDebug() << currentAdd;
    float stepMult = .001;
    currentStep = QVector3D(currentStep.x() - currentAdd.x() * stepMult,currentStep.y() - currentAdd.y() * stepMult, currentStep.z() - currentAdd.z() * stepMult * .01);
...
QVector<Derivable> Descent::AutoDiff(TestModel *originalModel, TestModel *finalModel, QVector3D transform)
{
    QVector<Derivable> res = {Derivable(), Derivable(), Derivable()};
    QVector<QPair<Derivable, Derivable>> orig = originalModel->toDerivableVector(),
                                         final = finalModel->toDerivableVector();
...
float Descent::CurrentDist(TestModel *originalModel, TestModel *finalModel, QVector3D derTransform)
{
    ...
    return Descent::CurDist(originalModel->toFloatVector(), finalModel->toFloatVector(), derVector);
}
